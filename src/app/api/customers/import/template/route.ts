...
export async function GET(request: NextRequest) {
  try {
    ...
    const headers = [
      ...
      'cartao_completo', // Card Complete (SIM/NÃO)
    ...
    // Add validation for cartao_completo column (G)
    for (let row = 2; row <= 1000; row++) {
      const cellAddress = XLSX.utils.encode_cell({ r: row - 1, c: 6 }) // Column G (cartao_completo)
      if (!worksheet[cellAddress]) worksheet[cellAddress] = { t: 's', v: '' }
    }
    ...
    const instructionsData = [
      ...
      ['- selos_atuais: Número de selos que o cliente já possui (0 a 999)'],
      ['- cartao_completo: SIM se o cartão está completo, NÃO caso contrário'],
